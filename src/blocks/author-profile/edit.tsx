...
import type { AuthorProfileAttributes } from '../../types/blocks';
...
interface EditProps {
	...
	setAttributes: (attributes: Partial<AuthorProfileAttributes>) => void;
	clientId?: string;
}
...
export default function Edit({ attributes, setAttributes }: EditProps) {
	...
	const blockStyles = {
		...
		transform:
			(transformScale && transformScale !== 1) || transformRotate
				? `scale(${ transformScale || 1 }) rotate(${ transformRotate || 0 }deg)`
				: undefined,
		filter:
			(filterBrightness && filterBrightness !== 100) ||
		...
		'--author-name-font-family': googleFont
			? `'${ googleFont }', sans-serif`
			: undefined,
		// Description styles
