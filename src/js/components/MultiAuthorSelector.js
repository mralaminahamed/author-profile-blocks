...
const MultiAuthorSelector = ({ 
    ...
    const handleRemoveAuthor = (authorIdToRemove) => {
        const newSelectedIds = selectedIds.filter(id => id !== authorIdToRemove);
        onSelectAuthors(newSelectedIds);
    };
                ...
                <>
                    <div className="apbl-author-select-controls">
                        ...
                        <Button
                            ...
                            disabled={!authorId || availableAuthors.length === 0}
                        >
                            {buttonText}
                        </Button>
                    </div>
                        ...
                        <div className="apbl-selected-authors">
                            ...
                            <ul className="apbl-selected-authors-list">
                                {selectedAuthors.map((author) => (
                                    <li key={author.id} className="apbl-selected-author-item">
                                        <span className="apbl-author-name">{author.name}</span>
                                        <Button
                                            variant="link"
                                            isDestructive
